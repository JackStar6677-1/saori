...
```mermaid
graph TD
    subgraph Interfaces ["🌐 Interfaces & Omnicanalidad"]
        WA["📱 WhatsApp Bot (Baileys v2)"]
        DC["💬 Discord Bot (Discord.js v14)"]
        MC["🎮 Minecraft Server (Pterodactyl & Sockets)"]
    end

    subgraph Seguridad ["🛡️ Escudo de Seguridad & Filtros"]
        Phish["🚨 Anti-Phishing & Links"]
        Flood["⏱️ Rate Limiting & Anti-Flood"]
        Raid["⛔ Anti-Mass Mentions"]
        Sanitize["🧼 Input & Prompt Sanitizer"]
    end
    ...
    subgraph Cerebro ["🧠 Núcleo Cognitivo SAORI (:8089)"]
        ...
        STT["👂 Audio Transcriber (Whisper / SpeechRec)"]
        IMG["🎨 Image Engine (Codex + Pollinations)"]
        Mem["💾 Memoria Rolling OpenClaw"]
    end

    subgraph Modulos ["⚡ Módulos Autónomos en Discord"]
        Tickets["🎫 Modals & Tickets Engine"]
        Stats["📊 Live Minecraft & Discord Stats"]
        Roles["🎭 Auto-Roles & Country Sync"]
        StaffMod["✨ Small Caps & Staff Suffix Auto-Sync"]
    end

    subgraph SRE ["🤖 Infraestructura & Flota SRE"]
        Triad["🤖 Trinidad de Agentes (Claude Code | Codex | Antigravity)"]
